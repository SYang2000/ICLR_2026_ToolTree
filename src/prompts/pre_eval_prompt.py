...
PRE_EVAL_SYSTEM_PROMPT: str = (
    ...
    "- are not grounded in the current context or fail to address the immediate sub-goal;\n"
    "- are irrelevant to the user's query or do not contribute meaningfully to the task.\n\n"
    "Important. Do not simulate the tool output; you are judging only the "
    "promised usefulness of this tool call as the next action. Strongly penalize candidates that duplicate any prior tool call in the trajectory with no clear new benefit, and candidates whose tool name does not match the user's stated intent."
)
